	...
	$(document).one('click','button[type="button"]', function(event){
		...
		db.transaction(function(transaction){
			...
			transaction.executeSql(sql,undefined,function(){
				alert("Datos eliminados exitosamente, Por favor refresque la tabla")
		...
		transaction.executeSql(sql,undefined, function(transaction,result){
			if(result.rows.length){
				...
				for(var i=0; i<result.rows.length; i++){
					...
					precio+' MXN$</span></td><td><button type="button" id="A'+id+'" class="btn btn-success" onclick="editar()"><img src="librerias/img/edit.png" /></button></td><td><button type="button" id="A'+id+'" class="btn btn-danger" onclick="eliminarRegistro()"><img src="librerias/img/delete.png" /></button></td></tr>');
				}
			}else{
...
$("#borrarTodo").click(function(){
	if(!confirm("¿Está seguro de borrar la tabla?, los datos se borraran definitivamente",""))
		return;
	db.transaction(function(transaction){
		var sql="DROP TABLE productos";
		transaction.executeSql(sql,undefined,function(){
			alert("Tabla eliminada exitosamente, actualice la página")
		}, function(transaction, err){
			alert(err.message);
		})
	})
